...
class Interpreter {
    interpret(node) {
        const functions = {};
        const expressions = [];
        const assignments = node.children.filter((child)=>child.type === 'assignment'
        );
        ...
        return async (...args)=>expressions.reduce(async (a, f)=>f({
                    functions
                }, ...await a)
            , args)
        ;
    ...
    array(node) {
        ...
        return _builtins.internals.array(...expressions);
    }
    operator(node) {
        const ops = node.children.filter((_, i)=>i % 2
        );
